...
function CustomPieTooltip({ active, payload }: any) {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900/95 dark:bg-slate-950/95 backdrop-blur-sm border border-slate-800 text-white px-3.5 py-2 rounded-xl shadow-xl text-xs space-y-1">
    ...
    return (
      <div className="rounded-2xl bg-white dark:bg-slate-900/90 p-6 shadow-sm border border-slate-200/80 dark:border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">Despesas por Categoria</h2>
          <span className="text-xs text-slate-400 dark:text-slate-500">Distribuição</span>
        </div>
        <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-12">Nenhuma despesa registrada ainda.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-slate-900/90 p-6 shadow-sm border border-slate-200/80 dark:border-slate-800 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">Despesas por Categoria</h2>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Distribuição percentual de saídas</p>
        </div>
          ...
          <Legend
            formatter={(value, entry: any) => (
              <span className="text-xs text-slate-600 dark:text-slate-300 font-medium">
                {value} <span className="text-slate-400 dark:text-slate-500 font-normal">({entry.payload.percentage}%)</span>
              </span>
            )}
            iconType="circle"
            iconSize={8}
