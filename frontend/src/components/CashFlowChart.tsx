...
function CustomTooltip({ active, payload, label }: any) {
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900/95 backdrop-blur-sm border border-slate-800 text-white px-3.5 py-2.5 rounded-xl shadow-xl text-xs space-y-1.5">
    ...
    return (
      <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200/80">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700">Fluxo de Caixa Mensal</h2>
          <span className="text-xs text-slate-400">Histórico</span>
        </div>
        <p className="text-sm text-slate-400 text-center py-12">Nenhum dado disponível ainda.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200/80 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700">Fluxo de Caixa Mensal</h2>
          <p className="text-xs text-slate-400 mt-0.5">Comparativo de entradas e saídas</p>
        </div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: -15, bottom: 0 }} barGap={6}>
          ...
          <Legend
            iconType="circle"
            iconSize={8}
            wrapperStyle={{ paddingTop: '12px', fontSize: '12px', color: '#64748b' }}
          />
